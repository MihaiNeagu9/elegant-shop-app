...
Another solution is Context, which allows global information to be shared between components. This way, any component in the tree can read or modify the information. I created the context using the createContext function from the react module, provided all components with the information through the value attribute, used the use hook (React 19+) and useContext hook to connect the context to the components, and linked the context to the state. The downside is that if I have multiple contexts or states to manage, then the main component (App component) will become bloated. So I separated the code from the root component into the context component. To connect State and Context, I used the useReducer hook.  
